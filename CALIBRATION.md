...
| `pontoon_y_m` | $y_{pont}$ | m | Lateral distance from vessel centreline to the waterline centroid of each pontoon. Used for **hydrostatics only** ($I_T$, transverse stability) | Tape measure from centreline to the middle of one pontoon at the waterline. Average port and starboard | Tape measure | 0.673 | |
| `motor_y_m` | $y_{mot}$ | m | 🔴 Lateral distance from vessel centreline to the thrust axis of each motor. This is the **lever arm** used in thrust allocation: $\tau_N = (T_S - T_P) \times y_{mot}$ | Measure the motor-to-motor separation with the vessel on land (plumb line from each motor shaft to the ground) and halve it. Measured separation: 1.53 m | Tape, plumb line | 0.765 | |
...
| `nrdot_frac` | $N_{\dot{r}}/I_{66}$ | — | 🔴 Yaw added moment fraction. $I_{zz,total} = I_{66,rigid}(1 + \|N_{rdot\_frac}\|)$. Since $K_p = I_{zz} \omega_n^2$, an error of 30% here translates to 30% wrong PID gains | **Free yaw decay test (recommended):** with vessel floating freely, apply a brief differential thrust impulse to spin the vessel to ~20–30°/s, then cut to zero. Record $r(t)$ from IMU. The **in-water** oscillation experiment gives $I_{zz,total}$ directly: $I_{zz,total} = N_{torque\_applied} / \alpha_{measured}$ for a known torque step, OR fit the free-yaw deceleration curve $(I_{zz,total}) \dot{r} = -(N_r r + N_{r^2}\|r\|r)$. Compare $I_{zz,total}$ to the in-air bifilar result to find $N_{rdot} = I_{zz,total} - I_{66,rigid}$. $N_{rdot\_frac} = N_{rdot}/I_{66,rigid}$ | IMU (gyro), data logging, propulsion control | -2.10 | |
...
> **Critical note on `motor_y_m`:** the allocation matrix inverse is $B^{-1} = (k_{pos} \begin{bmatrix}1 & 1\\ -l_1 & -l_2\end{bmatrix})^{-1}$ with $l_{1,2} = \mp y_{mot}$. An error in `motor_y_m` directly scales how much of the commanded yaw torque is actually applied — verify this measurement to ±5 mm.
...
| `max_thrust_per_motor_kgf` | $T_{max}$ | kgf | Maximum forward thrust per motor at full ESC command. Used to compute `TAU_MAX = 2 T_{max} g` and `N_MAX` | Thrust stand at full throttle. Read peak load cell value. Average over 10–30 s steady state to exclude transients. Manufacturer value is 11.5 kgf; **the bollard test on the actual propeller gives 9.5 kgf** | Load cell | 9.5 | |
| `max_reverse_thrust_per_motor_kgf` | $T_{max,rev}$ | kgf | Maximum reverse thrust per motor. Used to compute `N_MIN` | Thrust stand at full reverse throttle (bollard test) | Load cell | 5.5 | |
...
| `r_max_deg_s` | $\dot{\psi}_{max}$ | °/s | <!--�--> 🟡 Hard yaw-rate cap inside the reference model. The physical maximum at full differential thrust is ≈ 34 °/s, so the cap must stay below it for the reference to remain trackable | Keep at ≈70–80% of the measured physical maximum. Reduce further if the vessel overshoots its acceptance radius on tight waypoint turns | 25 | |
...
```
1. Geometry (tape measure, on land)           → motor_y_m, pontoon_y_m, length_m, beam_m, draft_m
...
`motor_y_m`, `k_pos`, `k_neg`, `max_speed_kn`, `xu_lin_frac`, `xu_quad_frac`, `nrdot_frac` / `izz_total_kgm2`.
...
|----------------|--------|
| `motor_y_m` wrong by 5 cm | Yaw torque per differential thrust error = 5/76.5 ≈ 7%. PID gains unchanged but effective plant gain is wrong → slight heading offset under turning | 
