...
class GNSSParser:
    ...
    def __init__(
        ...
        self.command_freq = (1/command_freq) # Frequency (Hz) to interval (seconds)
    ...
    def get_position(self):
        try:
            if self.gga_data:
                return {
                    'latitude': self.gga_data.latitude if self.gga_data.latitude else 0.0,
                    'longitude': self.gga_data.longitude if self.gga_data.longitude else 0.0,
                    'altitude': self.gga_data.altitude if self.gga_data.altitude else 0.0,
    ...
    def get_course_speed(self):
        try:
            if self.vtg_data:
                return {
                    'course': self.vtg_data.true_track if self.vtg_data.true_track else 0.0,
                    'speed_kmh': self.vtg_data.spd_over_grnd_kmph if self.vtg_data.spd_over_grnd_kmph else 0.0,
                }
        except Exception as e:
            print(f"Error getting course/speed: {e}")
    ...
    def get_time_date(self):
        try:
            ...
            print(f"Error getting time/date: {e}")
